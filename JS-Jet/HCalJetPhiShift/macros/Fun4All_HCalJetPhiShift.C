// Veronica Verkest January 2024
// -- single particle: produce single pions and
    ...
    bool single_particle = true)
{
  ...
  if (single_particle) {
    Enable::MBD = false;
    //   Enable::MBD_SUPPORT = true; // save hist in bbc support structure
...
//    G4MAGNET::magfield = "0."; // alternatively to specify a constant magnetic field, give a float number, which will be translated to solenoidal field in T, if string use as fieldmap name (including path)
  ...
  if (!single_particle) { Enable::GLOBAL_RECO = true; }
...
//    se->registerSubsystem(truthjetreco);
  ...
  if (single_particle) { caloPhiShift->SetSingleParticleMode(); }
  else { caloPhiShift->SetJetMode(); }
